...
class MinBisect:
    ...
    def _get_cut_depth(self, i, j, k, t):
        ...
        if t == 1:
            return self._get_val(j, k) - self._get_val(i, j) - self._get_val(i, k)
        elif t == 2:
            return self._get_val(i, k) - self._get_val(i, j) - self._get_val(j, k)
        elif t == 3:
            return self._get_val(i, j) - self._get_val(i, k) - self._get_val(j, k)
        else:  # t == 4:
            return self._get_val(i, j) + self._get_val(i, k) + self._get_val(j, k) - 2

    def _get_val(self, a, b):
        try:
            return self.v[a, b]
        except KeyError:
